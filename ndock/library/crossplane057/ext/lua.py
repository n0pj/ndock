# -*- coding: utf-8 -*-
from ..lexer import register_external_lexer
from ..builder import register_external_builder
from ..compat import fix_pep_479
from ..errors import NgxParserBaseException
from ..ext.abstract import CrossplaneExtension
...
class LuaBlockPlugin(CrossplaneExtension):
    ...
    def register_extension(self):
        ...
        register_external_builder(
            directives=self.directives, builder=self.build)
    ...
    def lex(self, char_iterator, directive):
        ...
        for char, line in char_iterator:
            ...
            if depth < 0:
                reason = 'unxpected "}"'
                raise LuaBlockParserSyntaxError(
                    reason, filename=None, lineno=line)

            if depth == 0:
                # True because this is treated like a string
                yield (token, line, True)
                yield (';', line, False)
                raise StopIteration
            token += char
