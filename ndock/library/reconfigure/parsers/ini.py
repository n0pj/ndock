...
from ..parsers.iniparse import INIConfig
...
class IniFileParser (BaseParser):
    ...
    def parse(self, content):
        content = '\n'.join(
            filter(None, [x.strip() for x in content.splitlines()]))
        if self.sectionless:
        ...
        for section in cp:
            ...
            for option in cp[section]:
                if option in cp[section]._options:
                    ...
                    node.comment = self._get_comment(
                        cp[section]._options[option])
        ...
        for section in tree.children:
            ...
            for option in section.children:
                if not isinstance(option, PropertyNode):
                    raise TypeError(
                        'Third level nodes should be PropertyNodes')
                cp[sectionname][option.name] = option.value
                if option.comment:
                    self._set_comment(
                        cp[sectionname]._options[option.name], option.comment)
