...
class IPTablesParser (BaseParser):
    ...
    def parse(self, content):
        content = filter(
            None, [x.strip() for x in content.splitlines() if not x.startswith('#')])
        root = RootNode()
        cur_table = None
        chains = {}
        for l in content:
                ...
                if tokens[0] == '-A':
                    ...
                    while tokens:
                        ...
                        while tokens and not tokens[0].startswith('-') and tokens[0] != '!':
                            option.append(
                                Node('argument', PropertyNode('value', tokens.pop(0))))
                        node.append(option)
                    ...
                    if item.name == 'append':
                        data += '-A %s %s%s\n' % (
                            ...
                            ' '.join(
                                ...
                                ' '.join(
                                    a.get('value').value for a in o.children if a.name == 'argument')
